...
function Analyzer(opts) {
  this.jwtClient = new googleapis.auth.JWT(opts.serviceEmail, null, opts.serviceKey, ["https://www.googleapis.com/auth/analytics", "https://www.googleapis.com/auth/analytics.readonly"]);
};
...
Analyzer.prototype.getAccountProfiles = function (opts) {
  ...
  var parseFunc = function (res) {
    return _.chain(res.items).map(function (item) {
      return {
        name: item.name,
        properties: _.chain(item.webProperties).map(function (webPro) {
